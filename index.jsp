...
<html>
<body>
<%-- Conditional Statements to detect what value is in the submit parameter This value is used to choose what query to execute when adding data to the database
...
<c:if test="${pageContext.request.method=='POST'}">
 <c:choose>
	  ...
	  <%--branch here if submit value is route then execute necessary sql query--%>  
...
INSERT INTO route (source,destination,distance) VALUES (?, ?, ?)
	  ...
	  <%--branch here if submit value is collection then execute necessary sql query--%>
<sql:update dataSource="${dataSource}" var="count">
...
<%--branch here if submit value is adddrver then execute necessary sql query--%>	  	  
<sql:update dataSource="${dataSource}" var="count">
INSERT INTO driver (name,busid) VALUES (?, ?)
 ...
 <c:when test="${param.submit=='AddBus'}">
	<%--branch here if submit value is addbus then execute necessary sql query--%>  
<sql:update dataSource="${dataSource}" var="count">
...
<%--Below, get parameter page which will be used in conditions that determine what is shown--%>
<%
...
<%-- declaration contains methods that print forms and menus for the different pages
Those ending with "Form", return a form, and those with "Menu" return a menu

It should be noted that the forms all need a parameter combo, that always contains the value for the current route. except the routeForm which adds a route 
--%>
<%!

 private String routeForm(){
   return("<fieldset><legend>Add new Route</legend><form method =\"POST\" action=\"\"> <label>Destination</label><input type=\"text\" name=\"destination\"/> <br/><label>Source</label><input type=\"text\" name=\"source\"/><br/> <label>Distance</label><input type=\"text\" name=\"distance\"/><br/> <input type=\"submit\" value=\"routes\" name=\"submit\"/><br/> </form></fieldset>");
...
<h3>Quick Links</h3>
<ul>
<%--print the menu according to what was in the page parameter defined previously--%>
...
<div id="main">
<%--use the page parameter as the page title--%>
<h2><%=current_page %></h2>
...
<div id="footer">
copyright &copy; 2012 designed by: Group.
</div>
<div class="clear"></div>
</div>
</body>
